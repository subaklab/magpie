...
import io.subak.connect.ConnectionAdapter;
import io.subak.connect.Link;
import io.subak.connect.SubakWifiLink;
import io.subak.connect.crtp.CommanderPacket;
import io.subak.joystick.DualJoystickView;
import io.subak.joystick.JoystickMovedListener;
...
public class MainActivity extends Activity {
    ...
    public boolean dispatchGenericMotionEvent(MotionEvent ev) {
        if ((ev.getSource() & InputDevice.SOURCE_CLASS_JOYSTICK) != 0
            ...
            right_analog_x = (float) (ev.getAxisValue(mRightAnalogXAxis));
            right_analog_y = (float) (ev.getAxisValue(mRightAnalogYAxis));
            left_analog_x = (float) (ev.getAxisValue(mLeftAnalogXAxis));
            left_analog_y = (float) (ev.getAxisValue(mLeftAnalogYAxis));

            split_axis_yaw_right = (float) (ev.getAxisValue(mSplitAxisYawRightAxis));
            split_axis_yaw_left = (float) (ev.getAxisValue(mSplitAxisYawLeftAxis));
...
//            	mv.startPlayback();
    ...
    private void linkConnect() {
        ...
        try {
            // create link
            mbugsLink = new SubakWifiLink("192.168.1.1", 2002);
